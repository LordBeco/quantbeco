...
def filter_dataframe_by_date(df, start_date, end_date):
    ...
    if not datetime_cols:
        st.warning(
            "⚠️ No valid datetime columns found for filtering. Showing all data.")
        return df
...
def enhanced_ai_strategy_builder_interface():
    ...
    st.markdown(
        "**Build complete trading strategies from natural language prompts**")
...
def strategy_generator_interface():
    ...
    with col1:
        ...
        selected_provider = st.selectbox(
            ...
            index=0 if Config.AI_PROVIDER == 'puter' else (
                1 if Config.AI_PROVIDER == 'openrouter' else 2),
        ...
        if selected_provider == 'openrouter':
            ...
            model_options = {
    model['id']: f"{model['name']} - {model['description']}" for model in available_models}
    ...
    with st.expander("⚙️ Advanced Generation Options", expanded=False):
        ...
        with col4:
            include_risk_management = st.checkbox(
                "Include Risk Management", value=True)
            include_position_sizing = st.checkbox(
                "Include Position Sizing", value=True)

        with col5:
            include_filters = st.checkbox(
    "Include Market Filters", value=False)
            include_time_filters = st.checkbox(
    "Include Time-based Filters", value=False)
    ...
    with col3:
        ...
        if st.session_state.get('generated_strategy'):
            export_format = st.selectbox(
                "Export As", ["Python", "Pine Script", "Both"])
    ...
    if generate_button and prompt.strip():
        ...
        try:
            ...
            if 'strategy_processor' not in st.session_state:
                if selected_provider == 'openrouter' and hasattr(
                    st.session_state, 'selected_openrouter_model'):
            ...
            status_text.text(
                f"🤖 Generating strategy with {selected_provider.upper()}...")

            # Process the enhanced prompt
            strategy_code = st.session_state.strategy_processor.process_prompt(
                enhanced_prompt)
            ...
            validation = st.session_state.strategy_processor.validate_strategy(
...
def indicators_builder_interface():
    ...
    "Indicator Name", placeholder="My Custom RSI")

        if indicator_category == "Trend":
            st.markdown("**Trend Indicators:**")
            trend_type = st.selectbox(
    "Type", [
        "Moving Average", "MACD", "ADX", "Parabolic SAR", "Custom"])
    ...
    "Period", min_value=1, max_value=200, value=20)
                source = st.selectbox(
    "Source", [
        "close", "open", "high", "low", "hl2", "hlc3", "ohlc4"])
                ...
                if st.button("Generate Moving Average Code"):
                    indicator_code = generate_moving_average_code(
                        ma_type, period, source, indicator_name)
            ...
            momentum_type = st.selectbox(
                "Type", ["RSI", "Stochastic", "CCI", "Williams %R", "Custom"])

            if momentum_type == "RSI":
                rsi_period = st.number_input(
    "RSI Period", min_value=2, max_value=50, value=14)
                rsi_source = st.selectbox(
                    "Source", ["close", "open", "high", "low"])
                overbought = st.number_input(
    "Overbought Level", min_value=50, max_value=100, value=70)
                oversold = st.number_input(
    "Oversold Level", min_value=0, max_value=50, value=30)

                if st.button("Generate RSI Code"):
                    indicator_code = generate_rsi_code(
    rsi_period, rsi_source, overbought, oversold, indicator_name)
                    st.session_state.custom_indicator_code = indicator_code
            ...
            if st.button("Generate Custom Indicator") and custom_description:
                with st.spinner("🤖 Generating custom indicator..."):
                    try:
                        ...
                        indicator_code = st.session_state.strategy_processor.generate_custom_indicator(
                            indicator_prompt)
                        st.session_state.custom_indicator_code = indicator_code
    ...
    with col2:
        ...
        if st.session_state.get('custom_indicator_code'):
            st.code(st.session_state.custom_indicator_code[:500] + "..." if len(
                st.session_state.custom_indicator_code) > 500 else st.session_state.custom_indicator_code, language="python")
    ...
    indicator_name, st.session_state.custom_indicator_code)
    ...
    st.session_state.custom_indicator_code)
        else:
    ...
    "Load Saved Indicator", [""] + saved_indicators)
...
def orders_risk_management_interface():
    ...
    st.markdown(
        "**Configure order types, position sizing, and risk management rules**")
    ...
    "Limit Offset (pips)",
    value=5.0,
     help="Offset from current price")
        elif entry_order_type == "Stop":
            stop_offset = st.number_input(
    "Stop Offset (pips)",
    value=10.0,
     help="Offset from current price")
    ...
    "Condition", [
        "RSI Level", "MACD Cross", "Price Level", "Custom"])
    ...
    "Stop Loss Type", [
        "Fixed Pips", "ATR Multiple", "Percentage", "Trailing"])
            if sl_type == "Fixed Pips":
                ...
                trailing_distance = st.number_input(
                    "Trailing Distance (pips)", value=15.0)

        use_take_profit = st.checkbox("Use Take Profit", value=True)
        if use_take_profit:
            tp_type = st.selectbox(
    "Take Profit Type", [
        "Fixed Pips", "Risk Reward Ratio", "Percentage", "Multiple Targets"])
    ...
    with col2:
        ...
        sizing_method = st.selectbox(
            ...
            ["Fixed Lot Size", "Fixed Percentage", "Kelly Criterion",
                "ATR-based", "Volatility-based", "Custom"],
    ...
    "Risk per Trade (%)", value=2.0, max_value=10.0)

        elif sizing_method == "Kelly Criterion":
            st.info(
                "Kelly Criterion will be calculated based on historical performance")
    ...
    "Max Daily Loss (%)", value=5.0, max_value=20.0)
        max_open_positions = st.number_input(
    "Max Open Positions", value=3, max_value=10)
        max_correlation = st.number_input(
    "Max Position Correlation", value=0.7, max_value=1.0)

        # Time-based filters
        use_time_filters = st.checkbox("Use Time Filters")
        if use_time_filters:
            trading_start = st.time_input(
    "Trading Start Time", value=datetime.strptime(
        "08:00", "%H:%M").time())
            trading_end = st.time_input(
    "Trading End Time", value=datetime.strptime(
        "17:00", "%H:%M").time())

            avoid_news = st.checkbox("Avoid News Events")
            if avoid_news:
                news_buffer = st.number_input(
                    "News Buffer (minutes)", value=30)
    ...
    if st.button("🔧 Generate Risk Management Code", type="primary"):
        with st.spinner("Generating risk management system..."):
            try:
                ...
                risk_management_code = generate_risk_management_code(
                    risk_config)
...
def pine_script_export_interface():
    ...
    "🤖 AI Refine", help="Use AI to improve the Pine Script")
    ...
    if st.session_state.get('pine_script_code'):
        ...
        try:
            pine_validation = st.session_state.pine_converter.validate_pine_script(
                st.session_state.pine_script_code)
            ...
            with col4:
                lines_count = len(
    st.session_state.pine_script_code.split('\n'))
        ...
        with col3:
            if st.button("🔗 Open TradingView"):
                st.markdown(
    "[Open TradingView Pine Editor](https://www.tradingview.com/pine-editor/)",
...
def display_generated_strategy():
    ...
    if strategy.metadata:
        with st.expander("📊 Strategy Information", expanded=False):
            ...
            with col1:
                st.write(
                    f"**Model Used:** {strategy.metadata.get('model_used', 'Unknown')}")
                st.write(
                    f"**Generated:** {datetime.fromtimestamp(strategy.metadata.get('generated_at', 0)).strftime('%Y-%m-%d %H:%M:%S')}")
            ...
            with col2:
                ...
                if strategy.entry_conditions:
                    st.write(
                        f"**Entry Conditions:** {len(strategy.entry_conditions)}")
                if strategy.exit_conditions:
                    st.write(
                        f"**Exit Conditions:** {len(strategy.exit_conditions)}")
...
def {name.lower().replace(' ', '_')}(data, period={period}, source='{source}'):
    ...
    if ma_type == 'SMA':
        ...
        return (data[source] * data['volume']).rolling(window=period).sum() / \
     ...
     stop_loss_price):
        ...
        if sizing_method == 'Fixed Lot Size':
            ...
            risk_amount = account_balance * \
                ({config.get('risk_percentage', 2.0)} / 100)
            pip_risk = abs(entry_price - stop_loss_price) * \
                           10000  # Assuming 4-digit pairs
...
def test_risk_management_rules(config):
    ...
    for i, scenario in enumerate(scenarios, 1):
        ...
        # Test against rules
        max_daily_loss = config.get(
    'max_daily_loss',
     5.0) / 100 * scenario['account_balance']
...
def analyze_strategy_complexity():
    ...
    if 'generated_strategy' in st.session_state:
        ...
        st.write(
            f"• Indicators: {len(strategy.indicators) if strategy.indicators else 0}")
        st.write(
            f"• Entry Conditions: {len(strategy.entry_conditions) if strategy.entry_conditions else 0}")
        st.write(
            f"• Exit Conditions: {len(strategy.exit_conditions) if strategy.exit_conditions else 0}")
    ...
    st.markdown("### 🤖 AI Strategy Builder")
    st.markdown(
        "**Convert your trading ideas into executable code using natural language**")
    ...
    model['id']: f"{model['name']} - {model['description']}" for model in available_models}
        ...
        if selected_provider != st.session_state.current_provider:
            ...
            # Clear existing strategy processor to reinitialize with new
            # provider
    ...
    with col2:
        ...
        if selected_provider == 'puter':
            ...
            if hasattr(st.session_state, 'selected_openrouter_model'):
                model_name = next(
    (m['name'] for m in available_models if m['id']
     == st.session_state.selected_openrouter_model),
     'Unknown')
    ...
    if selected_provider == 'puter':
        ...
        provider_valid = Config.OPENAI_API_KEY is not None and len(
            Config.OPENAI_API_KEY.strip()) > 0
    ...
    if selected_provider == 'puter':
        st.info(
            "🆓 **Using Puter AI** - Intelligent template-based strategy generation!")
    elif selected_provider == 'openrouter':
        model_name = "DeepSeek R1T2 Chimera"  # Default
        if hasattr(st.session_state, 'selected_openrouter_model'):
            model_name = next((m['name'] for m in available_models if m['id']
                              == st.session_state.selected_openrouter_model), model_name)
    ...
    with col3:
        ...
        selected_model = st.selectbox(
    "Model", model_options, index=0, help="Choose AI model")
    ...
    "🤖 AI Refine",
    value=True,
     help="Use AI to improve the Pine Script after conversion")
            ...
            with st.spinner(f"🌲 Converting to Pine Script ({conversion_method})..."):
                try:
                    if use_ai_refinement:
                        ...
                        st.success(
                            "✅ Pine Script conversion with AI refinement successful!")
                    else:
                        # Use basic rule-based conversion
                        pine_code = st.session_state.pine_converter.convert_to_pine(
                            strategy.python_code)
                        st.success("✅ Pine Script conversion successful!")
                    ...
                    st.write(
                        "• Check if your strategy has complex logic that needs manual adjustment")
                    st.write(
                        "• Pine Script conversion works best with simple indicator-based strategies")
    ...
    if st.session_state.generated_strategy:
        ...
        if st.session_state.generated_strategy.indicators or st.session_state.generated_strategy.entry_conditions:
            with st.expander("📋 Strategy Summary", expanded=False):
                ...
                if strategy.indicators:
                    st.write(
                        f"**Indicators Used:** {', '.join(strategy.indicators)}")
                ...
                if strategy.exit_conditions:
                    st.write(
                        f"**Exit Conditions:** {', '.join(strategy.exit_conditions)}")

                if strategy.metadata and 'original_prompt' in strategy.metadata:
                    st.write(
                        f"**Original Prompt:** {strategy.metadata['original_prompt'][:200]}...")
...
def analytics_dashboard():
    ...
    if data_source == "📁 Upload CSV File":
        ...
        if file:
            ...
            df_raw.columns = [c.lower().replace(" ", "_")
            ...
            if 'type' in df_raw.columns:
                balance_entries = df_raw[df_raw['type'].str.contains(
                ...
                if len(balance_entries) > 0:
                    ...
                    if len(positive_balances) > 0:
                        ...
                        balance_comment = positive_balances['comment'].iloc[
                            0] if 'comment' in positive_balances.columns else "Balance entry"
            ...
            if pnl_col_preview:
                ...
                df_raw_preview[pnl_col_preview] = pd.to_numeric(
                    df_raw_preview[pnl_col_preview].astype(
                        str).str.replace(',', ''),
                    errors="coerce"
                ).fillna(0)
                ...
                if 'type' in df_raw_preview.columns:
                    ...
                    trading_only_preview = trading_only_preview[
                        ~trading_only_preview['type'].str.contains(
                            'balance', case=False, na=False)
                    ]
                ...
                st.info(
                    f"📊 **Starting Balance Set**: ${starting_balance:,.2f}")

            # === TRANSACTION PROCESSING (DEPOSITS/WITHDRAWALS) ===
            st.markdown("### 🔍 Transaction Analysis")

            # Process broker statement to separate trading from non-trading
            # transactions
            ...
            if pnl_col_for_processing:
                ...
                df_raw[pnl_col_for_processing] = pd.to_numeric(
                    df_raw[pnl_col_for_processing].astype(
                        str).str.replace(',', ''),
                    errors="coerce"
                ).fillna(0)

                # Process transactions
                processed_data = process_broker_statement(
    df_raw, pnl_col_for_processing, starting_balance)
     ...
     processed_data['summary']['total_transactions'])
                    st.metric("Trading Transactions",
     processed_data['summary']['trading_transactions'])

                with col2:
                    st.metric("Deposits",
     processed_data['summary']['deposits'])
                    st.metric("Withdrawals",
     processed_data['summary']['withdrawals'])

                with col3:
                    net_deposits = processed_data.get('net_deposits', 0)
                    st.metric("Net Deposits", f"${net_deposits:,.2f}")
                    st.metric("Clean Trading P&L",
     f"${processed_data['clean_trading_pnl']:,.2f}")
                ...
                if processed_data['has_deposits_withdrawals'] and DEPOSIT_CHARTS_AVAILABLE:
                    ...
                    try:
                        comparison_chart = create_clean_vs_raw_equity_chart(
                            df_raw, df, pnl_col_for_processing)
                        st.plotly_chart(
    comparison_chart,
    use_container_width=True,
     key="equity_comparison_chart")
                    except Exception as e:
                        st.warning(
                            f"Could not create comparison chart: {str(e)}")

                    # Detailed deposit/withdrawal analysis
                    try:
                        dw_charts = create_deposit_withdrawal_analysis_charts(
                            processed_data)

                        if 'equity_comparison' in dw_charts:
                            st.plotly_chart(
    dw_charts['equity_comparison'],
    use_container_width=True,
     key="plotly_chart_1")
    ...
    use_container_width=True,
     key="plotly_chart_2")
                        ...
                        if 'performance_impact' in dw_charts:
                            with col2:
                                st.plotly_chart(
    dw_charts['performance_impact'],
    use_container_width=True,
     key="plotly_chart_3")
                    except Exception as e:
                        st.warning(
                            f"Could not create deposit/withdrawal charts: {str(e)}")
                    ...
                    st.warning(
                        "⚠️ Deposits/withdrawals detected but visualization charts not available. Analysis still uses clean trading data.")
                    st.info("""
                    📊 **Important**: All performance metrics below are calculated using **trading-only data**
                    (deposits and withdrawals excluded). This provides accurate trading performance analysis.
                    """)
            ...
            if 'type' in df.columns:
                balance_df = df[df['type'].str.contains(
                    'balance', case=False, na=False)].copy()
                # Don't filter out balance entries here since
                # transaction_handler already did it
            elif 'comment' in df.columns:
                balance_df = df[df['comment'].str.contains(
                    'Initial|balance', case=False, na=False)].copy()
                # Don't filter out balance entries here since
                # transaction_handler already did it

            # Clean numeric columns and calculate REAL PnL (including
        ...
        ',', ''), errors="coerce").fillna(0)
            ...
            if 'profit' in df.columns:
                ...
                with col2:
                    st.metric(
    "Total Commission",
     f"${df['commission'].sum():,.2f}" if 'commission' in df.columns else "$0.00")
                with col3:
                    st.metric(
    "Total Swaps",
     f"${df['swaps'].sum():,.2f}" if 'swaps' in df.columns else "$0.00")
                with col4:
                ...
                pnl = next(
    c for c in df.columns if "pnl" in c or "profit" in c)
                df[pnl] = pd.to_numeric(
    df[pnl].astype(str).str.replace(
        ',', ''), errors="coerce").fillna(0)
    ...
    balance_df[col].astype(str).str.replace(
        ',', ''), errors="coerce").fillna(0)

            # Calculate starting balance
            if not balance_df.empty:
                balance_amount = balance_df['profit'].iloc[0] if 'profit' in balance_df.columns else balance_df[list(
                    balance_df.columns)[0]].iloc[0]
                if balance_amount > 0:
        ...
        with col1:
            ...
            tl_password = st.text_input(
    "🔒 TradeLocker Password", type="password")

        with col2:
            tl_server = st.text_input(
    "🖥️ Server Name",
    value="GATESFX",
     help="e.g., GATESFX, LIVE, DEMO")
            tl_account_id = st.text_input(
    "🆔 Account ID",
    value="1691721",
     help="Your TradeLocker account ID")

        col3, col4, col5 = st.columns(3)
        with col3:
            tl_is_live = st.selectbox(
    "🔴 Account Type", [
        False, True], format_func=lambda x: "Demo" if not x else "Live")
        with col4:
            tl_acc_num = st.selectbox(
    "🔢 Account Number",
    [
        1,
        2,
        ...
        5],
        index=1,
         help="Select which account (1st, 2nd, 3rd, etc.) if you have multiple accounts")
        with col5:
            tl_days = st.number_input(
    "📅 Days of History",
    min_value=1,
    max_value=365,
     value=90)
            ...
            if tl_email and tl_password and tl_server and tl_account_id:
                with st.spinner("🔄 Connecting to TradeLocker..."):
                    try:
                        ...
                        connection_test = test_tradelocker_connection(
    tl_email, tl_password, tl_server, tl_account_id, tl_acc_num, tl_is_live)
    ...
    "Account #", account_info['account_number'])
                            ...
                            with col4:
                                st.metric(
    "Type", "Live" if account_info['is_live'] else "Demo")
                            with col5:
                                balance_value = balance_info.get(
                                    'balance', 0.0)
                                currency = balance_info.get('currency', 'USD')
                                st.metric(
    "Balance", f"${balance_value:,.2f} {currency}")

                            # Fetch trading data
                            with st.spinner("📊 Fetching trading history..."):
                                try:
                                    df = fetch_tradelocker_data(
    tl_email, tl_password, tl_server, tl_account_id, tl_acc_num, tl_is_live, tl_days)

                                    if df.empty:
                                        st.warning(
                                            f"⚠️ No trading history found for the last {tl_days} days.")
                                        st.info("💡 **Possible reasons:**")
                                        st.write(
                                            "• No trades executed in the selected time period")
                                        st.write(
                                            "• Different API endpoint needed for your broker")
                                        st.write(
                                            "• Historical data may be stored differently")

                                        # Suggest trying different periods
                                        st.write("**Try:**")
                                        st.write(
                                            "• Increase the 'Days of History' to 180 or 365")
                                        st.write(
                                            "• Check if you have trades in your TradeLocker platform")
                                        st.write(
                                            "• Contact your broker about API access to historical data")
                                    else:
                                        st.success(
                                            f"✅ Fetched {len(df)} trades from the last {tl_days} days")
    ...
    list(
        df.columns))
                                    ...
                                    st.error(
                                        f"❌ Error fetching trading data: {str(fetch_error)}")
                                    st.info("💡 **Debug Information:**")
                                    st.write(
                                        "• Connection was successful but data fetching failed")
                                    st.write(
                                        "• This might be due to different API endpoints for historical data")
                                    st.write(
                                        "• Try uploading a CSV export from your TradeLocker platform instead")
                        else:
                            st.error(
                            ...
                            with st.expander("🔍 Debug Information", expanded=False):
                                ...
                                st.write(
                                    "1. **Check Credentials**: Verify email and password in TradeLocker platform")
                                st.write(
                                    "2. **Verify Account ID**: Check your account ID in the trading platform")
                                st.write(
                                    "3. **Try Different Account Number**: If you have multiple accounts, try 1, 2, or 3")
                                st.write(
                                    "4. **Check Server Name**: Ensure server name matches your broker (e.g., GATESFX)")
                                st.write(
                                    "5. **Account Type**: Make sure Demo/Live selection matches your account")

                    except Exception as e:
                        st.error(f"❌ Error: {str(e)}")
            else:
                st.warning(
                    "⚠️ Please fill in all required fields: Email, Password, Server Name, and Account ID")
     ...
     "All Time"]
        ...
        with col1:
            ...
            if selected_quick:
                ...
                default_index = period_options.index(
                    st.session_state.get('period_selection', 'All Time'))
                selected_period = st.selectbox(
    "Select Time Period", period_options, index=default_index)
    ...
    selected_period, custom_start, custom_end)
        ...
        if start_date and end_date:
            if start_date == end_date:
                ...
                st.info(
                    f"📊 **Analyzing**: {selected_period} ({start_date} to {end_date})")
            ...
            if len(df) == 0:
                st.error(
                    f"❌ **No trades found** in the selected period ({start_date} to {end_date})")
                st.stop()
            elif len(df) < len(original_df):
                st.success(
                    f"✅ **Filtered**: {len(df)} trades out of {len(original_df)} total trades")
        else:
     ...
     "This Year"]
            ...
            for period in periods_to_analyze:
                ...
                if start_p and end_p:
                    period_df = filter_dataframe_by_date(
                        original_df_for_summary, start_p, end_p)
                    if len(period_df) > 0:
                        period_pnl = period_df[pnl].sum()
                        period_trades = len(period_df)
                        win_rate = (
    period_df[pnl] > 0).sum() / len(period_df) * 100
    ...
    use_container_width=True,
     hide_index=True)
        ...
        if start_date and end_date and selected_period != "All Time":
            ...
            # We need the original unfiltered dataframe to calculate cumulative
            # PnL
            period_starting_balance = compute_period_starting_balance(
                original_df, df, pnl, starting_balance)
            top_kpis = compute_top_kpis(
    df, pnl, starting_balance_override=period_starting_balance)

            # Show additional context for filtered periods
            if len(df) > 0:
                first_trade_date = df.iloc[0]['close_time_parsed'].date(
                ) if 'close_time_parsed' in df.columns else "Unknown"
                last_trade_date = df.iloc[-1]['close_time_parsed'].date(
                ) if 'close_time_parsed' in df.columns else "Unknown"

                # Calculate how much PnL was accumulated before this period
                trades_before_count = original_df.index.get_loc(
            ...
            top_kpis = compute_top_kpis(
    df, pnl, starting_balance_override=starting_balance)
        ...
        current_equity_for_kelly = top_kpis.get(
            'Current Equity', starting_balance)
        # Extract numeric value from the formatted string
        if isinstance(current_equity_for_kelly, str):
            current_equity_for_kelly = float(
    current_equity_for_kelly.replace(
        '$', '').replace(
            ',', ''))
        ...
        with col2:
            ...
            st.write(
                f"• Traditional rate: {user_current_lots / (current_equity_for_kelly / 1000):.3f} per 1k")
            st.write(f"• Standard rate: 0.020 per 1k")

        kelly_metrics = compute_kelly_metrics(
    df, pnl, current_equity_for_kelly, user_current_lots)
        ...
        if selected_period != "All Time" and start_date and end_date:
            ...
            with col2:
                ...
                period_days = (end_date - start_date).days + \
                               1 if start_date != end_date else 1

                # Show period starting balance calculation for transparency
                if 'original_df' in locals() and len(df) < len(original_df):
                    trades_before_period = len(original_df) - len(df) - (len(
                        original_df) - original_df.index[-1] - 1) + (df.index[0] if len(df) > 0 else 0)
                    st.write(
                        f"• **Period Starting Balance**: ${period_starting_balance:,.2f}")
                    st.write(
                        f"• **Trades Before Period**: {original_df.index.get_loc(df.index[0]) if len(df) > 0 else 0}")
                ...
                if period_days > 0:
                    st.write(
                        f"• **Avg PnL/Day**: ${period_pnl/period_days:.2f}")
                if period_trades > 0:
                    st.write(
                        f"• **Avg PnL/Trade**: ${period_pnl/period_trades:.2f}")
        else:
            kpi_cols = st.columns(4)
            for col, (k, v) in zip(kpi_cols, top_kpis.items()):
                col.metric(k, v)
        ...
        st.markdown(
            "**Optimal position sizing based on your trading history and current equity**")
        ...
        if kelly_metrics.get('has_kelly_data', False):
            ...
            with col1:
                ...
                if edge > 0:
                    st.success(
                        f"📈 **Statistical Edge**: ${edge:.2f} per trade")
                else:
                    st.error(
                        f"📉 **No Edge**: ${edge:.2f} per trade (negative expectancy)")
            ...
            with col2:
                ...
                st.write(
                    f"• **Your Traditional Method**: {traditional_lots:.3f} lots (0.02 per 1k)")
    ...
    traditional_lots) /
    traditional_lots *
     100) if traditional_lots > 0 else 0

                if difference_pct > 10:
                    st.write(
                        f"• **Difference**: +{difference_pct:.1f}% (Kelly suggests larger size)")
                elif difference_pct < -10:
                    st.write(
                        f"• **Difference**: {difference_pct:.1f}% (Kelly suggests smaller size)")
                else:
                    st.write(
                        f"• **Difference**: {difference_pct:.1f}% (Similar sizing)")
    ...
    use_container_width=True,
     key="plotly_chart_7")
            ...
            st.info(
                "Insufficient trading data for Kelly Criterion calculation. Need both winning and losing trades.")
    ...
    use_container_width=True,
     key="plotly_chart_8")
        ...
        if edge_status == 'NO_EDGE':
            st.error(
                "🚨 **CRITICAL: NO STATISTICAL EDGE** - Strategy has no edge (expectancy ≤ 0)")
        elif edge_status == 'EDGE_DECAY':
            st.error(
                "⚠️ **WARNING: EDGE DECAY DETECTED** - Expectancy is deteriorating")
        elif edge_status == 'EDGE_IMPROVING':
            st.success(
                "✅ **POSITIVE: EDGE STRENGTHENING** - Expectancy is improving")
        elif edge_status == 'EDGE_STABLE':
            st.info("📊 **NEUTRAL: EDGE STABLE** - Performance appears consistent")
        elif edge_status == 'INSUFFICIENT_DATA':
            st.warning(
                "⚠️ **INSUFFICIENT DATA** - Need more trades for edge analysis")
        else:
    ...
    risk_metrics.get(
        ...
        # Check if decision_rules exists (might not exist with insufficient
        # data)
        if 'decision_rules' in edge_analysis:
            ...
            st.warning(
                "⚠️ **Insufficient data for professional decision rules**")
            st.info(
                "Need at least 20 trades in the selected period for meaningful edge analysis.")
        ...
        if missing_analytics:
            with st.expander("🔴 Missing Analytics - Next Level Analysis Needed", expanded=False):
                st.markdown(
                    "**To reach TradingView-grade analytics, you still need:**")
                for missing in missing_analytics:
                    st.markdown(f"• {missing}")

                st.markdown("---")
                st.markdown(
                    "**💡 Pro Tip:** These missing metrics separate professional traders from retail traders. Without them, you're trading half-blind.")
        else:
        ...
        st.markdown(
            "**See at one glance which how many days you are making or losing money. Click a day to look at the trades.**")
        ...
        if len(df) > 0:
            ...
            for col in df.columns:
                if any(word in col.lower()
                       for word in ['time', 'date', 'datetime', 'close_time']):
                    try:
            ...
            if datetime_col and original_col:
                ...
                if len(available_dates) > 0:
                    ...
                    with col1:
                        ...
                        default_year_idx = available_years.index(
    st.session_state.calendar_year) if st.session_state.calendar_year in available_years else 0
                        selected_year = st.selectbox(
    "📅 Year", available_years, index=default_year_idx, key="calendar_year_select")
                    ...
                    with col2:
                        month_names = [calendar.month_name[i]
                            for i in range(1, 13)]
                        month_numbers = list(range(1, 13))
                        default_month_idx = st.session_state.calendar_month - 1
                        selected_month_name = st.selectbox(
    "📅 Month", month_names, index=default_month_idx, key="calendar_month_select")
                        selected_month = month_numbers[month_names.index(
                            selected_month_name)]
    ...
    "➡️ Next Month",
     key="calendar_next_month"):
                    ...
                    # Create calendar with selected month/year (using ORIGINAL
                    # full dataset, not filtered)
                    calendar_chart = create_daily_calendar_chart(
    original_df,
    pnl,
    ...
    use_container_width=True,
     key="plotly_chart_9")
        ...
        if len(df) > 0 and 'datetime_col' in locals(
        ) and datetime_col and 'original_col' in locals():
            col1, col2, col3 = st.columns(3)

            # Calculate daily stats for the selected month using ORIGINAL
            # dataset
            ...
            # Filter for selected month/year from calendar (not main app
            # filter)
            if 'selected_year' in locals() and 'selected_month' in locals():
                month_filter = (
    df_copy[datetime_col].dt.year == selected_year) & (
        df_copy[datetime_col].dt.month == selected_month)
    ...
    "Avg Trades/Day", f"{avg_trades_per_day:.1f}")
                            st.metric(
    "Month Total P&L", f"${total_month_pnl:.2f}")
                else:
                    # Show message if no data for selected month
                    st.info(
                        f"📅 No trading data found for {calendar.month_name[selected_month]} {selected_year}")
            else:
     ...
     key="plotly_chart_11")
        ...
        with col1:
            ...
            for hour, data in time_tables['worst_hours'].iterrows():
                if data['Total_PnL'] < 0:
                    st.write(
                        f"• **{hour}:00** - Loss: ${data['Total_PnL']:.0f} ({data['Trade_Count']:.0f} trades)")
        ...
        with col2:
            ...
            for hour, data in time_tables['best_hours'].iterrows():
                if data['Total_PnL'] > 0:
                    st.write(
                        f"• **{hour}:00** - Profit: ${data['Total_PnL']:.0f} ({data['Trade_Count']:.0f} trades)")
    ...
    use_container_width=True,
     key="plotly_chart_12")
        ...
        with col1:
            st.markdown(
                f"**🏆 Best Month:** {best_month.name[1]} (${best_month['Total_PnL']:.0f})")
            st.markdown(
                f"**📉 Worst Month:** {worst_month.name[1]} (${worst_month['Total_PnL']:.0f})")

        with col2:
            profitable_months = (monthly_stats['Total_PnL'] > 0).sum()
            total_months = len(monthly_stats)
            st.markdown(
                f"**📊 Monthly Win Rate:** {profitable_months}/{total_months} ({profitable_months/total_months*100:.0f}%)")
        ...
        col1, col2, col3, col4 = st.columns(4)
        with col1:
            st.metric("Max Consecutive Losses",
     f"{risk_data['max_consecutive_losses']}")
        with col2:
            st.metric("Avg Drawdown Duration",
     f"{risk_data['avg_drawdown_duration']:.1f} trades")
        with col3:
        ...
        # Risk charts
        dd_chart, recovery_chart, consec_chart = risk_pain_charts(
            risk_data, df)
    ...
    use_container_width=True,
     key="plotly_chart_15")
        ...
        if risk_data['max_consecutive_losses'] > 5:
            st.error(
                f"🚨 **HIGH PAIN RISK**: Max consecutive losses ({risk_data['max_consecutive_losses']}) may exceed psychological tolerance")

        if risk_data['avg_drawdown_duration'] > 10:
            st.warning(
                f"⚠️ **LONG RECOVERY PERIODS**: Average drawdown lasts {risk_data['avg_drawdown_duration']:.1f} trades")
        ...
        st.markdown(
            "**Deep dive into lot sizing, risk-reward, pip analysis, and directional bias**")
     ...
     key="plotly_chart_16")
            ...
            if trading_insights['lot_analysis']['has_lot_data']:
                ...
                with col1:
                    st.metric(
    "Avg Lot Size",
     f"{lot_stats['avg_lot_size']:.2f}")
                with col2:
                    st.metric(
    "Lot Consistency",
     f"{lot_stats['lot_consistency']:.1%}")
                with col3:
                    st.metric(
    "Min/Max Lots",
     f"{lot_stats['min_lot_size']:.2f} / {lot_stats['max_lot_size']:.2f}")
                with col4:
        ...
        if 'direction_analysis' in insights_charts:
            ...
            st.plotly_chart(
    insights_charts['direction_analysis'],
    use_container_width=True,
     key="plotly_chart_17")
            ...
            if trading_insights['direction_analysis']['has_direction_data']:
                ...
                display_direction['Total_PnL'] = display_direction['Total_PnL'].apply(
                    lambda x: f"${x:,.2f}")
                display_direction['Avg_PnL'] = display_direction['Avg_PnL'].apply(
                    lambda x: f"${x:.2f}")
                display_direction['Win_Rate'] = display_direction['Win_Rate'].apply(
                    lambda x: f"{x:.1f}%")
                display_direction['Profit_Factor'] = display_direction['Profit_Factor'].apply(
                    lambda x: f"{x:.2f}")
                display_direction['Consistency'] = display_direction['Consistency'].apply(
                    lambda x: f"{x:.1%}")

                # Rename columns for display
                display_direction.columns = [
    'Total PnL',
    'Avg PnL',
    'Trades',
    'PnL Std',
    ...
    'Profit Factor',
     'Consistency']
                display_direction = display_direction[[
                    'Total PnL', 'Avg PnL', 'Trades', 'Win Rate', 'Profit Factor', 'Consistency']]
     ...
     key="plotly_chart_18")
            ...
            if trading_insights['rr_analysis']['has_rr_data']:
                ...
                with col1:
                    st.metric(
    "Avg R:R Ratio",
     f"{rr_stats['avg_rr_ratio']:.2f}:1")
                with col2:
                    st.metric("Best R:R", f"{rr_stats['best_rr']:.2f}")
                with col3:
                    st.metric(
    "RR Consistency",
     f"{rr_stats['rr_consistency']:.1%}")
        ...
        if pip_chart:
            ...
            if trading_insights['pip_analysis'].get('estimated', False):
                calculation_method = trading_insights['pip_analysis'].get(
                    'calculation_method', 'estimated')
                if calculation_method == 'symbol_aware':
                    st.info(
                        "💡 **Pip calculation**: Symbol-aware calculation (NAS100=points, XAUUSD=0.1 moves, JPY=0.01 moves, Forex=0.0001 moves)")
                else:
                    st.info(
                        "💡 **Pip calculation**: Estimated from price data (assuming standard forex pairs)")
    ...
    use_container_width=True,
     key="plotly_chart_19")
            ...
            with col1:
                st.metric(
    "Avg Pips/Trade",
     f"{pip_stats['avg_pips_per_trade']:.1f}")
            with col2:
                st.metric("Total Pips", f"{pip_stats['total_pips']:.0f}")
            with col3:
                st.metric(
    "Best Trade",
     f"{pip_stats['best_pip_trade']:.1f} pips")
            with col4:
                st.metric(
    "Worst Trade",
     f"{pip_stats['worst_pip_trade']:.1f} pips")

        if 'symbol_analysis' in insights_charts:
            st.markdown("#### 🎯 Symbol Performance Analysis")
            st.plotly_chart(
    insights_charts['symbol_analysis'],
    use_container_width=True,
     key="plotly_chart_20")
            ...
            if trading_insights['symbol_analysis']['has_symbol_data']:
                ...
                with col1:
                    ...
                    for symbol, data in top_symbols.iterrows():
                        st.write(
                            f"• **{symbol}**: ${data['Total_PnL']:.2f} ({data['Trade_Count']:.0f} trades, {data['Win_Rate']:.1f}% WR)")
        ...
        if 'position_sizing' in insights_charts:
            ...
            st.plotly_chart(
    insights_charts['position_sizing'],
    use_container_width=True,
     key="plotly_chart_21")
    ...
    win_loss_pie(
        ...
        st.markdown("---")
        ai_diagnosis = comprehensive_ai_diagnosis(
    df, metrics, risk_data, time_data, pnl, selected_period)

        # Executive Summary
        executive_summary = generate_executive_summary(
            ai_diagnosis, metrics, selected_period)
        ...
        if 'symbol_analysis' in ai_diagnosis and 'symbol_performance' in ai_diagnosis[
            'symbol_analysis']:
            ...
            if not symbol_perf.empty:
                ...
                display_df['Total_PnL'] = display_df['Total_PnL'].apply(
                    lambda x: f"${x:,.2f}")
                display_df['Avg_PnL'] = display_df['Avg_PnL'].apply(
                    lambda x: f"${x:.2f}")
                display_df['Win_Rate'] = display_df['Win_Rate'].apply(
                    lambda x: f"{x:.1f}%")
                display_df['Max_DD_Pct'] = display_df['Max_DD_Pct'].apply(
                    lambda x: f"{x:.1f}%")
    ...
    'Max Drawdown',
     'Expectancy']
                ...
                with col1:
                    ...
                    for insight in symbol_insights[:len(
                        symbol_insights) // 2 + 1]:
                        st.write(f"• {insight}")
        ...
        with col1:
            ...
            st.write(
                f"• Average Rolling Expectancy: **${avg_rolling_exp:.2f}**")
...
def data_upload_interface():
    ...
    col for col in required_columns if col not in data.columns]
                    ...
                    if not missing_columns:
                        st.success(
                            "✅ **Basic validation passed** - All required columns present")
                    else:
                ...
                with st.expander("📊 Data Statistics", expanded=False):
                    try:
                        ...
                        if 'close' in data.columns:
                            st.write(
                                f"• Price range: {data['close'].min():.4f} to {data['close'].max():.4f}")
                        if 'timestamp' in data.columns:
                            st.write(
...
def backtest_configuration_interface():
    ...
    with col1:
        strategy_source = st.selectbox(
            ...
            ["AI Generated Strategy (Recommended)",
                                     "Upload Python File", "Custom Code"],
        ...
        if strategy_source == "AI Generated Strategy (Recommended)":
            if st.session_state.get('generated_strategy'):
                ...
                with st.expander("📋 Strategy Details", expanded=False):
                    st.write(
                        f"**Indicators:** {', '.join(strategy.indicators) if strategy.indicators else 'None detected'}")
                    st.write(
                        f"**Entry Conditions:** {len(strategy.entry_conditions) if strategy.entry_conditions else 0}")
                    st.write(
                        f"**Exit Conditions:** {len(strategy.exit_conditions) if strategy.exit_conditions else 0}")
                    st.write(
                        f"**Code Length:** {len(strategy.python_code)} characters")
                    st.write(
                        f"**Generated by:** {strategy.metadata.get('provider', 'Unknown')} - {strategy.metadata.get('model_used', 'Unknown')}")

                    st.markdown("**Code Preview:**")
                    st.code(strategy.python_code[:500] + "..." if len(
                        strategy.python_code) > 500 else strategy.python_code, language="python")
            else:
    ...
    "Upload strategy file (.py)", type=['py'])
            if strategy_file:
                ...
                with st.expander("📋 Strategy Preview", expanded=False):
                    st.code(strategy_code[:500] + "..." if len(strategy_code)
                            > 500 else strategy_code, language="python")
            else:
...
def apply_risk_management(data, signals):
    ...
    with col2:
        ...
        if strategy_source == "AI Generated Strategy" and st.session_state.get(
            'generated_strategy'):
            strategy = st.session_state.generated_strategy
            if strategy.indicators:
    ...
    "Kelly Fraction", value=0.25, max_value=1.0)
    ...
    "Custom Timezone", placeholder="America/New_York")
            ...
            if start_date <= end_date:
                ...
                st.info(
                    f"📊 **Selected Period**: {len(period_data):,} data points from {start_date} to {end_date}")
            else:
    ...
    "Execution Delay (ms)", value=0, help="Simulated execution delay")
            ...
            max_drawdown_stop = st.number_input(
    "Max Drawdown Stop (%)",
    value=20.0,
     help="Stop trading at this drawdown")
            daily_loss_limit = st.number_input(
    "Daily Loss Limit (%)", value=5.0, help="Stop trading for the day")

        with col2:
            st.markdown("**Performance Tracking:**")
            track_equity_curve = st.checkbox("Track Equity Curve", value=True)
            track_drawdown = st.checkbox("Track Drawdown", value=True)
            track_trade_details = st.checkbox(
                "Track Trade Details", value=True)

            st.markdown("**Output Options:**")
            generate_report = st.checkbox(
    "Generate Detailed Report", value=True)
            export_trades = st.checkbox("Export Trade List", value=True)
            create_charts = st.checkbox(
    "Create Performance Charts", value=True)
    ...
    "Configuration Name",
     placeholder="My Backtest Config")
...
def run_backtest_interface():
    ...
    if not st.session_state.get('backtest_config'):
        st.warning(
            "⚠️ **Configure backtest settings** in the 'Configuration' tab")
        return
    ...
    with col1:
        ...
        st.metric(
    "Instrument",
     f"{config['instrument_name']} ({config['instrument_type']})")
    ...
    "Save Intermediate Results", value=False)

    with col2:
        st.markdown("**Estimated Time:**")
        data_points = len(
            period_data) if 'period_data' in locals() else len(data)
...
def backtest_results_interface():
    ...
    if 'trades' in results:
        ...
        with col2:
            ...
            long_trades = len(
                trades_df[trades_df['direction'] == 'long']) if 'direction' in trades_df.columns else 0
            short_trades = len(
                trades_df[trades_df['direction'] == 'short']) if 'direction' in trades_df.columns else 0
    ...
    chart_tabs = st.tabs(["📈 Equity & Drawdown",
    "📊 Monthly Returns",
    "🎯 Trade Distribution",
     "📅 Calendar"])
    ...
    drawdown_chart,
    use_container_width=True,
     key="detailed_drawdown_curve")

    with chart_tabs[1]:
        if 'monthly_returns' in results:
            monthly_chart = create_monthly_returns_chart(
    ...
    with chart_tabs[2]:
        if 'trades' in results:
            distribution_chart = create_trade_distribution_chart(
                results['trades'])
            if distribution_chart:
                st.plotly_chart(
    distribution_chart,
    use_container_width=True,
     key="trade_distribution_chart")
            else:
                st.info(
                    "📊 Trade distribution chart not available - insufficient trade data")
...
def trading_charts_interface():
    ...
    if not has_backtest_data and not has_tick_data:
        ...
        st.write(
            "• Or use the main Analytics Dashboard for broker statement analysis")
        return
     ...
     key="price_action_chart")
        ...
        with st.expander("📊 Chart Controls", expanded=False):
            ...
            with col2:
                ...
                show_trade_labels = st.checkbox(
                    "Show Trade Labels", value=False)
...
def smart_read_tick_data(uploaded_file):
    ...
    try:
        ...
        try:
            ...
            try:
                # Try to parse parts as numbers (skip first part which might be
                # a date string)
                ...
                has_headers = False
                st.info(
                    "🔍 **No headers detected** - First line contains numeric data")
    ...
    delimiter=delimiter,
     header=None)
            ...
            if len(data.columns) == 5:
                ...
                st.success(
                    "✅ **Auto-assigned**: timestamp, open, high, low, close")
    ...
    'volume',
     'extra']
                st.success(
                    "✅ **Auto-assigned**: timestamp, open, high, low, close, volume, extra")
            elif len(data.columns) == 4:
                # Sometimes data might be: timestamp, high, low, close (no
                # open)
                ...
                st.success(
                    "✅ **Auto-assigned**: timestamp, high, low, close (open created from close)")
            else:
                st.warning(
                    f"⚠️ **Unexpected format**: {len(data.columns)} columns detected")
                st.info(
                ...
                if len(data.columns) >= 5:
                    ...
                    if len(data.columns) > 5:
                        new_columns.extend(
                            [f'extra_{i}' for i in range(len(data.columns) - 5)])
                    ...
                    data.columns = [
                        f'column_{i}' for i in range(len(data.columns))]
                    st.error(
                        f"❌ **Insufficient columns**: Only {len(data.columns)} found, need at least 2 (timestamp + price)")
                    return None
        ...
        for col in numeric_columns:
            if col in data.columns:
                try:
                    ...
                    st.warning(
                        f"⚠️ **Could not convert {col} to numeric**: {str(e)}")
    ...
    subset=[
        ...
        if initial_rows != final_rows:
            st.warning(
                f"⚠️ **Removed {initial_rows - final_rows} rows with missing data**")
        ...
        st.success(
            f"✅ **Data processing complete**: {len(data)} rows, {len(data.columns)} columns")
...
def validate_tick_data(data):
    ...
    if all(col in data.columns for col in ['open', 'high', 'low', 'close']):
        try:
            ...
            for idx, row in data.iterrows():
                ...
                # High should be >= max(open, close) and Low should be <=
                # min(open, close)
            ...
            if ohlc_issues > 0:
                validation_results['warnings'].append(
                    f"OHLC logic issues in {ohlc_issues} rows (High < max(O,C) or Low > min(O,C))")
            else:
                validation_results['info'].append(
            ...
            validation_results['warnings'].append(
                f"Could not validate OHLC relationships: {str(e)}")

    # Check for missing values
    missing_values = data.isnull().sum().sum()
    if missing_values > 0:
        validation_results['warnings'].append(
            f"Found {missing_values} missing values across all columns")
        ...
        if len(missing_cols) > 0:
            validation_results['info'].append(
                f"Missing values by column: {dict(missing_cols)}")
    else:
    ...
    if 'timestamp' in data.columns and len(data) > 1:
        try:
            ...
            # Find the most common time difference (likely the intended
            # frequency)
            ...
            if len(mode_diff) > 0:
                ...
                if large_gaps > 0:
                    validation_results['warnings'].append(
                        f"Found {large_gaps} potential data gaps (>2x expected frequency)")
                else:
                    validation_results['info'].append(
                        "Data continuity looks good")

                validation_results['info'].append(
                    f"Detected frequency: {expected_freq}")

        except Exception as e:
            validation_results['warnings'].append(
                f"Could not analyze data continuity: {str(e)}")
        ...
        try:
            ...
            if not close_prices.empty:
                ...
                if price_range > price_mean * 10:
                    validation_results['warnings'].append(
                        "Extremely wide price range detected - check for data errors")

                # Check for zero or negative prices
                invalid_prices = (close_prices <= 0).sum()
                if invalid_prices > 0:
                    validation_results['warnings'].append(
                        f"Found {invalid_prices} zero or negative prices")

                validation_results['info'].append(
                    f"Price range: {close_prices.min():.4f} to {close_prices.max():.4f}")

        except Exception as e:
            validation_results['warnings'].append(
                f"Could not validate price ranges: {str(e)}")
...
def display_validation_results(validation_results):
    ...
    if validation_results['errors']:
        ...
        if any(
    "Missing required columns" in error for error in validation_results['errors']):
            st.info("💡 **Solutions for missing columns:**")
            st.write(
                "• **No headers detected**: The system will auto-assign column names based on data structure")
            st.write(
                "• **Tab-separated data**: Make sure your file uses tabs (\\t) as separators")
            st.write(
                "• **Expected format**: timestamp, open, high, low, close [, volume]")
            st.write(
                "• **Minimum requirement**: timestamp + close price (other OHLC values will be created)")
    ...
    if validation_results['warnings']:
        ...
        if any(
    "OHLC logic issues" in warning for warning in validation_results['warnings']):
        ...
        if any(
    "data gaps" in warning for warning in validation_results['warnings']):
            st.info(
                "💡 **Data Gaps**: Missing time periods detected. This is normal for markets that close (weekends, holidays).")

        if any(
    "missing values" in warning for warning in validation_results['warnings']):
            st.info(
                "💡 **Missing Values**: Some data points are missing. These will be handled automatically during backtesting.")
    ...
    total_checks = len(validation_results['errors']) + len(
        validation_results['warnings']) + len(validation_results['info'])
    if total_checks > 0:
        ...
        if quality_score >= 90:
            st.success(
                f"📊 **Data Quality Score**: {quality_score}/100 (Excellent)")
...
def run_comprehensive_backtest(config, data, execution_mode, show_progress):
    ...
    with st.spinner("🚀 Initializing backtest engine..."):
        ...
        try:
            ...
            st.success(
                f"✅ Backtest initialized with {len(filtered_data):,} data points")
    ...
    try:
        ...
        if 'generated_strategy' in st.session_state and st.session_state.generated_strategy:
            ...
            st.info(
                "💡 Please generate a strategy in the AI Strategy Builder tab or upload/paste strategy code")
            return
        ...
        try:
            ...
            if 'calculate_indicators' in strategy_globals:
                strategy_data = strategy_globals['calculate_indicators'](
                    strategy_data)
            ...
            if 'generate_signals' in strategy_globals:
                signals_result = strategy_globals['generate_signals'](
                    strategy_data)
                if isinstance(
    signals_result,
     pd.DataFrame) and 'signal' in signals_result.columns:
                    ...
                    if 'signal' not in strategy_data.columns:
                        st.warning(
                            "⚠️ Strategy did not generate signals, using fallback")
                        strategy_data['signal'] = 0
            else:
                # Fallback: look for signal column in data
                if 'signal' not in strategy_data.columns:
                    st.warning(
                        "⚠️ No generate_signals function found, using fallback")
                    # Simple moving average fallback
                    strategy_data['sma_fast'] = strategy_data['close'].rolling(
                        10).mean()
                    strategy_data['sma_slow'] = strategy_data['close'].rolling(
                        20).mean()
                    strategy_data['signal'] = 0
                    strategy_data.loc[strategy_data['sma_fast']
                        > strategy_data['sma_slow'], 'signal'] = 1
                    strategy_data.loc[strategy_data['sma_fast']
                        < strategy_data['sma_slow'], 'signal'] = -1

            # Apply risk management if available
            if 'apply_risk_management' in strategy_globals:
                risk_result = strategy_globals['apply_risk_management'](
                    strategy_data, strategy_data[['signal']])
            ...
            st.write(
                "• Required functions (calculate_indicators, generate_signals) may be missing")
        ...
        strategy_data['strategy_returns'] = strategy_data['signal'].shift(
            1) * strategy_data['returns']

        # Calculate equity curve
        starting_balance = config['starting_balance']
        strategy_data['equity'] = starting_balance * \
            ...
            # Exit logic
                ...
                if position == 1:  # Long position
                    pnl = (exit_price - entry_price) / entry_price * \
                           starting_balance * config['base_lot_size']
                    direction = 'long'
                else:  # Short position
                ...
                # Apply costs
                pnl -= config['spread'] * config['pip_value'] * \
                    config['base_lot_size']  # Spread cost
                pnl -= config['commission'] * \
                    config['base_lot_size']  # Commission
        ...
        total_return = (
            strategy_data['equity'].iloc[-1] / starting_balance - 1) * 100
        max_drawdown = (
    (strategy_data['equity'] / strategy_data['equity'].cummax()) - 1).min() * 100
    ...
    total_trades *
     100) if total_trades > 0 else 0
        ...
        avg_win = np.mean([t['pnl'] for t in trades if t['pnl']
                          > 0]) if winning_trades > 0 else 0
        avg_loss = np.mean(
    ...
    if name in configs:
        ...
        for key, value in config.items():
            if key.endswith('_date') and isinstance(value, str):
                ...
                st.session_state[f'loaded_{key}'] = datetime.fromisoformat(
                    value).date()
            else:
...
def create_trading_calendar_chart(trades):
    ...
    try:
        ...
        pnl_col = 'pnl' if 'pnl' in df.columns else (
...
def backtesting_engine_interface():
        ...
        st.markdown(
            "**Test your trading strategies against historical tick data**")
        ...
        def _smart_read_csv(uploaded_file):
            ...
            has_headers = False
            try:
                # Try to parse each part as a number (skip first part which
                # might be timestamp)
            ...
            if has_headers:
                ...
                st.success(
                    f"✅ **Using existing headers**: {list(data.columns)}")
            else:
                data = pd.read_csv(
    uploaded_file,
    delimiter=delimiter,
    ...
    'timestamp', 'open', 'high', 'low', 'close']
                        ...
                        data.columns = [
                            f'column_{i}' for i in range(len(data.columns))]
                        st.error(
                            f"❌ **Insufficient columns**: Only {len(data.columns)} found, need at least 5")
    ...
    col for col in required_columns if col not in data.columns]
                ...
                if missing_columns:
                    ...
                    st.write(
                        "• Check if your CSV uses the correct delimiter (comma, tab, semicolon)")
                    ...
                    with st.expander("📊 Data Information", expanded=False):
                        ...
                        if missing_data.sum() > 0:
                            ...
                            for col, count in missing_data.items():
                                if count > 0:
                                    st.write(
                                        f"  • {col}: {count} ({count/len(data)*100:.1f}%)")
                        else:
                        ...
                        with col1:
                            if 'timestamp' in data.columns:
                                ...
                                try:
                                    ...
                                    st.write(
                                        f"Duration: {timestamps.max() - timestamps.min()}")
                                except:
        ...
        if st.session_state.uploaded_data is not None:
            ...
            st.info(
                "💡 **Note:** This is a simplified backtesting interface. Full backtesting engine features are under development.")
            ...
            with col2:
                ...
                selected_strategy = st.selectbox(
                    "Strategy Source", strategy_options)

                if selected_strategy == "AI-Generated Strategy (Recommended)":
                    if 'generated_strategy' in st.session_state and st.session_state.generated_strategy:
                        st.success(
                            "✅ AI-generated strategy ready for backtesting")
                        ...
                        with st.expander("📋 Strategy Details", expanded=False):
                            ...
                            st.write(
                                f"**Code Length:** {len(strategy.python_code)} characters")
                            st.write(
                                f"**Generated:** {strategy.metadata.get('provider', 'Unknown')} - {strategy.metadata.get('model_used', 'Unknown')}")
                    else:
                        st.warning("⚠️ No AI-generated strategy found")
                        st.info(
                            "💡 Generate a strategy in the **AI Strategy Builder** tab first")
                        st.write("**Steps:**")
                        st.write("1. Go to AI Strategy Builder tab")
                    ...
                    if 'custom_strategy_code' in st.session_state and st.session_state.custom_strategy_code:
                        st.success(
                            "✅ Custom strategy code ready for backtesting")
                        st.info(
                            "💡 Your custom strategy will be executed during backtesting")
                    else:
                        st.warning("⚠️ No strategy code available")
                        st.info(
                            "💡 Please provide strategy code via AI Builder, file upload, or custom code")
            ...
            if st.button("🚀 Run Simple Backtest", type="primary"):
                ...
                try:
                    ...
                    # Execute the user's strategy code (from AI Strategy
                    # Builder)
                    if selected_strategy == "AI-Generated Strategy (Recommended)":
                        if 'generated_strategy' not in st.session_state or not st.session_state.generated_strategy:
                            ...
                            st.info(
                                "� Please geenerate a strategy in the AI Strategy Builder tab first")
                            return

                        progress_bar.progress(40)
                        status_text.text(
                            "🤖 Executing AI-generated strategy...")
                        ...
                        try:
                            ...
                            # The strategy should have defined functions like
                            # calculate_indicators, generate_signals
                            if 'calculate_indicators' in strategy_globals:
                                data = strategy_globals['calculate_indicators'](
                                    data)
    ...
    signals_result,
     pd.DataFrame) and 'signal' in signals_result.columns:
                            ...
                            st.write(
                                "• Required functions (calculate_indicators, generate_signals) may be missing")
                            st.write(
                                "• Data column names may not match expected format")
                            st.code(f"Error details: {str(e)}")
                            return

                    else:
                        # For simple backtesting, we should still use the AI-generated strategy
                        # but provide fallback options if no strategy is
                        # available
                        if 'generated_strategy' in st.session_state and st.session_state.generated_strategy:
                            # Use the AI-generated strategy
                            progress_bar.progress(40)
                            status_text.text(
                                "🤖 Executing your AI-generated strategy...")
                            ...
                            try:
                                ...
                                if 'generate_signals' in strategy_globals:
                                    signals_result = strategy_globals['generate_signals'](
                                        data)
                                    if isinstance(
    signals_result, pd.DataFrame) and 'signal' in signals_result.columns:
                                        data['signal'] = signals_result['signal']
                                    elif isinstance(signals_result, pd.Series):
                                        data['signal'] = signals_result
                                    else:
                                ...
                                st.warning(
                                    f"⚠️ Could not execute AI strategy: {str(e)}")
                                st.info(
                                    "Using simple moving average fallback...")
                                ...
                                data['sma_slow'] = data['close'].rolling(
                                    20).mean()
                                data['signal'] = 0
                                data.loc[data['sma_fast'] >
                                    data['sma_slow'], 'signal'] = 1
                                data.loc[data['sma_fast'] <
                                    data['sma_slow'], 'signal'] = -1
                        else:
                            st.warning(
                                "⚠️ No AI-generated strategy found. Using simple moving average fallback.")
                            # Simple fallback strategy
                    ...
                    # Calculate equity curve
                    data['equity'] = starting_balance * \
                        (1 + data['strategy_returns']).cumprod()

                    # Basic performance metrics
                    total_return = (
                        data['equity'].iloc[-1] / starting_balance - 1) * 100
                    max_drawdown = (
    (data['equity'] / data['equity'].cummax()) - 1).min() * 100
...
def analyze_drawdown_periods(drawdown_curve):
    ...
    if not drawdown_curve:
        return pd.DataFrame(
            {'Period': ['No data'], 'Duration': ['N/A'], 'Max DD': ['N/A']})
