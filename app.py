...
def analytics_dashboard():
    ...
    if df is not None and not df.empty:
        ...
        st.markdown("**Simulate thousands of possible future equity curves based on your trading history**")
        ...
        with col2:
            simulation_periods = st.number_input(
                "Future Periods", 
                min_value=10, 
                max_value=1000, 
                value=len(df),
                step=10,
                help="Number of future trades to simulate"
            )
        ...
        with col3:
            confidence_levels = st.multiselect(
                ...
                [50, 68, 80, 90, 95, 99],
                default=[68, 95],
                help="Statistical confidence bands to display"
            )
        
        with col4:
            sample_paths_to_show = st.number_input(
                "Sample Paths to Display",
                min_value=10,
                max_value=500,
                value=50,
                step=10,
                help="Number of individual simulation paths to show on chart (for visual clarity)"
            )
        ...
        with col1:
            ...
            st.write(f"• Win Rate: {(df[pnl] > 0).mean()*100:.1f}%")
            st.write(f"• Avg P&L: ${df[pnl].mean():.2f}")
        
        with col2:
            st.markdown("**Simulation Info:**")
            st.write(f"• All {num_simulations:,} simulations used for statistics")
            st.write(f"• Only {sample_paths_to_show} paths shown for clarity")
            st.write(f"• Confidence bands include ALL simulations")
            ...
            with st.spinner(f"Running {num_simulations:,} simulations..."):
                try:
                    ...
                    mc_results = run_monte_carlo_simulation(
                        df, pnl, num_simulations, simulation_periods, 
                        starting_balance, confidence_levels
                    )
                    
                    # Add sample paths configuration to results
                    mc_results['sample_paths_to_show'] = sample_paths_to_show
                    
                    # Store results in session state
                    st.session_state.monte_carlo_results = mc_results
                    
                    st.success(f"✅ Completed {num_simulations:,} simulations successfully!")
                    
                except Exception as e:
                    st.error(f"❌ Monte Carlo simulation failed: {str(e)}")
                    st.info("💡 This might be due to insufficient trade data or extreme outliers")
        ...
        if st.session_state.get('monte_carlo_results'):
            ...
            with col1:
                final_median = mc_results['percentiles'][50][-1]
                st.metric("Median Final Balance", f"${final_median:,.2f}")
            ...
            with col3:
                ...
                st.metric("Risk of Ruin", f"{prob_ruin:.1f}%")
            
            with col4:
                max_expected = mc_results['percentiles'][95][-1]
                st.metric("95% Upside", f"${max_expected:,.2f}")
            
            with col5:
                min_expected = mc_results['percentiles'][5][-1]
                st.metric("5% Downside", f"${min_expected:,.2f}")
            
            # Monte Carlo chart
            st.markdown("#### 📈 Monte Carlo Simulation Chart")
            mc_chart = create_monte_carlo_chart(mc_results, df, pnl, starting_balance)
            if mc_chart:
            ...
            with col1:
                ...
                for conf in sorted(confidence_levels, reverse=True):
                    if conf > 50:
                        final_value = mc_results['percentiles'][conf][-1]
                        gain = ((final_value - starting_balance) / starting_balance) * 100
                        st.write(f"• **{conf}th percentile**: ${final_value:,.2f} (+{gain:.1f}%)")
                
                st.markdown("**🎯 Expected Outcomes:**")
                expected_return = ((final_median - starting_balance) / starting_balance) * 100
                st.write(f"• **Expected Return**: {expected_return:.1f}%")
                st.write(f"• **Expected Final Balance**: ${final_median:,.2f}")
            
            with col2:
                st.markdown("**📉 Downside Scenarios:**")
                for conf in sorted([c for c in confidence_levels if c < 50]):
                    final_value = mc_results['percentiles'][conf][-1]
                    loss = ((starting_balance - final_value) / starting_balance) * 100
                    st.write(f"• **{conf}th percentile**: ${final_value:,.2f} (-{loss:.1f}%)")
                
                st.markdown("**⚠️ Risk Metrics:**")
                st.write(f"• **Probability of Loss**: {100-prob_profit:.1f}%")
                st.write(f"• **Risk of Ruin**: {prob_ruin:.1f}%")
                if mc_results.get('max_drawdown_stats'):
                    max_dd = mc_results['max_drawdown_stats']['median']
                    st.write(f"• **Expected Max DD**: {max_dd:.1f}%")
            
            # Monte Carlo insights
            st.markdown("#### 💡 Monte Carlo Insights")
            
            insights = generate_monte_carlo_insights(mc_results, df, pnl, starting_balance)
            
            for insight in insights:
                if "🚨" in insight or "HIGH RISK" in insight:
                    st.error(insight)
                elif "⚠️" in insight or "WARNING" in insight:
                    st.warning(insight)
                elif "✅" in insight or "GOOD" in insight:
                    st.success(insight)
                else:
                    st.info(insight)
            ...
            with col1:
                if st.button("📊 Download Simulation Data"):
                    mc_csv = export_monte_carlo_results(mc_results)
                    st.download_button(
                        "📥 Download CSV",
                        mc_csv,
                        f"monte_carlo_simulation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        "text/csv"
                    )
            
            with col2:
                if st.button("📈 Download Chart Data"):
                    chart_csv = export_monte_carlo_chart_data(mc_results)
                    st.download_button(
                        "📥 Download Chart CSV",
                        chart_csv,
                        f"monte_carlo_chart_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                        "text/csv"
                    )
...
def backtest_results_interface():
    ...
    with col3:
        if st.button("📊 Broker Statement Export"):
            # Export in exact broker statement format
            if 'trades' in results:
                broker_csv = generate_broker_statement_csv(results['trades'])
                st.download_button(
                    "📥 Download Broker Statement",
                    broker_csv,
                    f"broker_statement_{datetime.now().strftime('%Y-%m-%d_%H-%M')}.csv",
                    "text/csv",
                    help="Export trades in exact broker statement format"
                )
            else:
                st.warning("⚠️ No trade data available for export")
    
    # === TRADE VISUALIZATION ===
    if 'trades' in results and st.session_state.get('uploaded_tick_data') is not None:
        st.markdown("##### 📈 Trade Visualization")
        
        # Create candlestick chart with trade entry/exit points
        trade_chart = create_trade_chart_with_entries_exits(
            st.session_state.uploaded_tick_data, 
            results['trades']
        )
        
        if trade_chart:
            st.plotly_chart(trade_chart, use_container_width=True, key="trade_visualization_chart")
        else:
            st.info("📊 Trade visualization requires both tick data and completed trades")
    ...
    with col1:
        if st.button("📊 Detailed Analysis CSV"):
            if 'trades' in results:
                detailed_csv = generate_detailed_analysis_csv(results)
                st.download_button(
    ...
    with col3:
        if st.button("📈 View in Analytics"):
            # Convert backtest results to analytics format and switch tabs
            if 'trades' in results:
                analysis_data = convert_backtest_to_analysis_format(results)
                st.session_state.converted_backtest_data = analysis_data
                st.info("✅ Backtest data converted. Switch to Analytics Dashboard to view.")


def generate_broker_statement_csv(trades_data):
    """Generate CSV in exact broker statement format"""
    try:
        # Convert trades data to broker statement format
        broker_trades = []
        
        for i, trade in enumerate(trades_data):
            # Generate unique ticket number
            ticket = f"742193218591{str(i).zfill(7)}"
            
            # Extract trade information
            symbol = trade.get('symbol', 'SYMBOL')
            trade_type = 'buy' if trade.get('direction', 'long') == 'long' else 'sell'
            lots = trade.get('size', 0.1)
            
            # Format timestamps
            open_time = trade.get('entry_time', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            close_time = trade.get('exit_time', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
            
            # Prices
            open_price = trade.get('entry_price', 0.0)
            close_price = trade.get('exit_price', 0.0)
            
            # Calculate profit
            profit = trade.get('profit', 0.0)
            
            # Standard broker fees
            commission = -0.7  # Standard commission
            swaps = 0  # No swap for short-term trades
            
            # Comment (reference to original trade)
            comment = f"742193218591{str(i+1000).zfill(7)}"
            ...
            broker_trades.append({
                ...
                'Lots': f"{lots:.8f}",
                'Open Time': open_time,
                'Open Price': f"{open_price:.2f}" if open_price else "0.00",
                'Close Time': close_time,
                'Close Price': f"{close_price:.2f}" if close_price else "0.00",
                'Swaps': swaps,
                'Commission': commission,
                'Profit': f"{profit:.2f}",
                'Comment': comment
        ...
        st.error(f"Error generating broker statement CSV: {str(e)}")
        return ""


def generate_detailed_analysis_csv(results):
    """Generate detailed analysis CSV with all metrics"""
    try:
        # Combine all results into a comprehensive CSV
        analysis_data = []
        ...
        analysis_data.append(performance_metrics)
        ...
        if 'trades' in results:
            for i, trade in enumerate(results['trades']):
                ...
                analysis_data.append(trade_data)
        
        df = pd.DataFrame(analysis_data)
        return df.to_csv(index=False)
        
    except Exception as e:
        st.error(f"Error generating detailed analysis CSV: {str(e)}")
        return ""


def create_trade_chart_with_entries_exits(tick_data, trades_data):
    """Create candlestick chart with trade entry/exit points"""
    try:
        if not PLOTLY_AVAILABLE:
            st.warning("⚠️ Plotly not available for charting")
            return None
        ...
        if not all(col in df.columns for col in required_cols):
            st.error("❌ Tick data missing required OHLC columns")
            return None
        ...
        for i, trade in enumerate(trades_data):
            try:
                ...
                # Entry point
                entry_color = '#00ff00' if direction == 'long' else '#ff0000'
                fig.add_trace(go.Scatter(
                    ...
                    name=f"Entry {i+1}",
                    hovertemplate=f"<b>Entry {i+1}</b><br>" +
                ...
                # Exit point
                exit_color = '#00aa00' if profit > 0 else '#aa0000'
                fig.add_trace(go.Scatter(
                    x=[exit_time],
                    y=[exit_price],
                    mode='markers',
                ...
                st.warning(f"⚠️ Could not plot trade {i+1}: {str(trade_error)}")
                continue
        
        # Update layout
        fig.update_layout(
            title="📈 Price Chart with Trade Entries & Exits",
            xaxis_title="Time",
            yaxis_title="Price",
        ...
        st.error(f"❌ Error creating trade chart: {str(e)}")
        return None


def convert_backtest_to_analysis_format(results):
    """Convert backtest results to format compatible with main analytics dashboard"""
    try:
        if 'trades' not in results:
            return pd.DataFrame()
        
        # Convert trades to broker statement format
        analysis_trades = []
        
        for trade in results['trades']:
            # Map backtest trade to analytics format
            analysis_trade = {
                'Ticket': f"BT_{trade.get('id', 0)}",
                'Symbol': trade.get('symbol', 'BACKTEST'),
                'Type': 'buy' if trade.get('direction', 'long') == 'long' else 'sell',
                'Lots': trade.get('size', 0.1),
                'Open Time': trade.get('entry_time', ''),
                'Open Price': trade.get('entry_price', 0),
                'Close Time': trade.get('exit_time', ''),
                'Close Price': trade.get('exit_price', 0),
                'Swaps': 0,
                'Commission': -0.7,
                'Profit': trade.get('profit', 0),
                'Comment': f"Backtest Trade {trade.get('id', 0)}"
            }
            analysis_trades.append(analysis_trade)
        
        return pd.DataFrame(analysis_trades)
        
    except Exception as e:
        st.error(f"Error converting backtest data: {str(e)}")
        return pd.DataFrame()
...
def run_comprehensive_backtest(config, data, execution_mode, show_progress):
    ...
    with st.spinner("🚀 Initializing backtest engine..."):
        ...
        try:
            ...
            st.info("🔧 Preprocessing tick data...")
            
            # Clean column names (remove spaces and normalize)
            data.columns = [col.strip().lower() for col in data.columns]
            st.info(f"✅ Cleaned column names: {list(data.columns)}")
            
            # Check if data has proper column names
            if len(data.columns) == 6 and str(data.columns[0]).isdigit():
                # Data has no headers, assign standard OHLCV names
                data.columns = ['timestamp', 'open', 'high', 'low', 'close', 'volume']
                st.info("✅ Assigned standard OHLCV column names")
            elif 'timestamp' not in data.columns:
                # Try to detect timestamp column
                timestamp_col = None
                for col in data.columns:
                    if any(word in str(col).lower() for word in ['time', 'date', 'datetime']):
                        timestamp_col = col
                        break
                ...
                if timestamp_col:
                    ...
                    st.info("✅ Using first column as timestamp")
            ...
            # If still missing OHLC, try to use available price columns
            if 'close' not in data.columns:
                price_cols = [col for col in data.columns if any(word in str(col).lower() for word in ['price', 'bid', 'ask', 'mid'])]
                if price_cols:
                    data['close'] = data[price_cols[0]]
                    st.info(f"✅ Using {price_cols[0]} as close price")
                else:
                    st.error("❌ Could not find price data. Please ensure your data has OHLC columns.")
                    return
            ...
            for col in ['open', 'high', 'low']:
                if col not in data.columns:
                    data[col] = data['close']
                    st.info(f"✅ Using close price for missing {col} column")
            
            # Add volume if missing
            if 'volume' not in data.columns:
                data['volume'] = 1
                st.info("✅ Added default volume column")
            ...
            st.info(f"📊 Data columns: {list(filtered_data.columns)}")
    ...
    try:
        ...
        # Execute the strategy code
        try:
            # Create a safe execution environment for strategy code
            strategy_globals = {
                'pd': pd,
                'np': np,
                'data': filtered_data.copy()
            ...
            # Add helper functions to handle pandas Series boolean operations
            ...
            def safe_vectorized_assignment(data, condition, column, value):
                """Safely assign values using vectorized operations"""
                try:
                    data.loc[condition, column] = value
                    return True
                except Exception as e:
                    print(f"Warning: Vectorized assignment failed: {str(e)}")
                    return False
            
            # Add safe condition helper to globals
            strategy_globals['safe_condition_check'] = safe_condition_check
            strategy_globals['safe_vectorized_assignment'] = safe_vectorized_assignment
            
            # Add pandas Series boolean operation warning
            strategy_globals['__pandas_warning__'] = """
            WARNING: Avoid using pandas Series directly in if statements.
            Use .any(), .all(), or vectorized operations instead.
            
            WRONG: if data['rsi'] < 30:
            RIGHT: if (data['rsi'] < 30).any():
            BEST:  data.loc[data['rsi'] < 30, 'signal'] = 1
            """
            
            # Execute the strategy code with error handling
            ...
            if 'calculate_indicators' in strategy_globals:
                try:
                    ...
                    st.warning(f"⚠️ Error in calculate_indicators: {str(indicator_error)}")
            ...
            if 'generate_signals' in strategy_globals:
                try:
                    ...
                    st.warning(f"⚠️ Error in generate_signals: {str(signal_error)}")
                ...
                try:
                    ...
                    if isinstance(risk_result, pd.DataFrame):
                        ...
                        if 'signal' in risk_result.columns:
                            ...
                            st.success("✅ Risk management applied successfully")
                except Exception as risk_error:
                    st.warning(f"⚠️ Error in apply_risk_management: {str(risk_error)}")
                    # Continue without risk management
            ...
            st.write("• **Pandas Series boolean operations** - Use .any(), .all(), or proper vectorized operations")
            st.code(f"Error details: {str(e)}")
...
def run_monte_carlo_simulation(df, pnl_col, num_simulations, num_periods, starting_balance, confidence_levels):
    ...
    try:
        ...
        trade_returns = df[pnl_col].values
        
        # Remove any NaN or infinite values
        trade_returns = trade_returns[np.isfinite(trade_returns)]
        
        if len(trade_returns) < 10:
            raise ValueError("Insufficient trade data for Monte Carlo simulation")
        ...
        simulations = np.zeros((num_simulations, num_periods + 1))
        simulations[:, 0] = starting_balance
        ...
        for sim in range(num_simulations):
            ...
            for period in range(num_periods):
                # Sample from historical returns (bootstrap method)
                if len(trade_returns) > 0:
                    sampled_return = np.random.choice(trade_returns)
                else:
                    # Fallback to normal distribution
                    sampled_return = np.random.normal(mean_return, std_return)
                
                current_balance += sampled_return
                simulations[sim, period + 1] = current_balance
        
        # Calculate percentiles
        percentiles = {}
        for conf in confidence_levels + [5, 50, 95]:  # Always include 5th, 50th, 95th
            percentiles[conf] = np.percentile(simulations, conf, axis=0)
        
        # Calculate additional statistics
        final_balances = simulations[:, -1]
        probability_of_profit = (final_balances > starting_balance).mean() * 100
        probability_of_ruin = (final_balances <= starting_balance * 0.1).mean() * 100  # 90% loss = ruin
        
        # Calculate maximum drawdown statistics
        max_drawdowns = []
        for sim in range(num_simulations):
            equity_curve = simulations[sim, :]
...
def create_monte_carlo_chart(mc_results, df, pnl_col, starting_balance):
    """Create Monte Carlo simulation chart similar to the attached image"""
    try:
        import plotly.graph_objects as go
        import numpy as np
        ...
        num_periods = mc_results['num_periods']
        
        # X-axis (trade numbers)
        x_axis = np.arange(num_periods + 1)
        
        # Add confidence bands (similar to the image)
        colors = ['rgba(255,0,0,0.1)', 'rgba(255,165,0,0.1)', 'rgba(0,255,0,0.1)', 
                 'rgba(0,0,255,0.1)', 'rgba(128,0,128,0.1)', 'rgba(255,192,203,0.1)']
        
        # Sort confidence levels for proper layering
        conf_levels = sorted(mc_results['confidence_levels'], reverse=True)
        ...
        for i, conf in enumerate(conf_levels):
            ...
            if upper_conf in percentiles and lower_conf in percentiles:
                color = colors[i % len(colors)]
                
                fig.add_trace(go.Scatter(
                    x=np.concatenate([x_axis, x_axis[::-1]]),
                    y=np.concatenate([percentiles[upper_conf], percentiles[lower_conf][::-1]]),
                    fill='toself',
                    fillcolor=color,
        ...
        # Add sample simulation paths (like the colorful lines in the image)
        sample_paths_to_show = mc_results.get('sample_paths_to_show', 50)  # Default to 50 if not specified
        sample_indices = np.random.choice(len(simulations), min(sample_paths_to_show, len(simulations)), replace=False)
        path_colors = ['rgba(255,0,0,0.3)', 'rgba(0,255,0,0.3)', 'rgba(0,0,255,0.3)', 
                      'rgba(255,165,0,0.3)', 'rgba(128,0,128,0.3)', 'rgba(0,255,255,0.3)',
                      'rgba(255,0,255,0.3)', 'rgba(128,128,0,0.3)', 'rgba(0,128,128,0.3)',
        ...
        for i, sim_idx in enumerate(sample_indices):
            ...
            fig.add_trace(go.Scatter(
                x=x_axis,
                y=simulations[sim_idx],
                mode='lines',
                line=dict(color=color, width=1),
                name=f'Sample Path ({sample_paths_to_show} of {mc_results["num_simulations"]:,})' if i == 0 else '',
                showlegend=True if i == 0 else False,
                hoverinfo='skip'
            ))
        
        # Add median line (like the blue line in the image)
        fig.add_trace(go.Scatter(
            x=x_axis,
            y=percentiles[50],
            mode='lines',
            ...
            hovertemplate='<b>Median Path</b><br>Trade: %{x}<br>Balance: $%{y:,.2f}<extra></extra>'
        ))
        
        # Add actual historical equity curve if available
        if len(df) > 0:
            historical_equity = np.concatenate([[starting_balance], starting_balance + df[pnl_col].cumsum().values])
            historical_x = np.arange(len(historical_equity))
            ...
            fig.add_trace(go.Scatter(
                ...
                name='Actual Historical Performance',
                hovertemplate='<b>Historical</b><br>Trade: %{x}<br>Balance: $%{y:,.2f}<extra></extra>'
            ))
        ...
        # Update layout to match the style of the attached image
        fig.update_layout(
            title={
                'text': f'Monte Carlo Simulation - {mc_results["num_simulations"]:,} Paths',
                'x': 0.5,
                'xanchor': 'center',
                'font': {'size': 16}
            },
        ...
        # Format y-axis to show currency (FIXED: update_yaxes instead of update_yaxis)
...
def generate_monte_carlo_insights(mc_results, df, pnl_col, starting_balance):
    ...
    try:
        ...
        # Risk assessment
        if prob_ruin > 20:
            insights.append("🚨 **HIGH RISK OF RUIN**: Over 20% chance of losing 90% of capital")
        elif prob_ruin > 10:
            insights.append("⚠️ **MODERATE RISK**: 10-20% chance of significant losses")
        elif prob_ruin > 5:
            insights.append("⚠️ **LOW-MODERATE RISK**: 5-10% chance of major drawdown")
        else:
            insights.append("✅ **LOW RISK**: Less than 5% chance of catastrophic loss")
        
        # Probability assessment
        if prob_profit > 80:
            insights.append("✅ **HIGH PROBABILITY OF SUCCESS**: Over 80% chance of profit")
        elif prob_profit > 60:
            insights.append("✅ **GOOD PROBABILITY**: 60-80% chance of profit")
        elif prob_profit > 40:
            insights.append("⚠️ **MODERATE PROBABILITY**: 40-60% chance of profit")
        else:
            insights.append("🚨 **LOW PROBABILITY**: Less than 40% chance of profit")
        
        # Expected return assessment
        if expected_return > 50:
            insights.append("✅ **EXCELLENT EXPECTED RETURN**: Median projection shows >50% gains")
        elif expected_return > 20:
            insights.append("✅ **GOOD EXPECTED RETURN**: Median projection shows 20-50% gains")
        elif expected_return > 0:
            insights.append("✅ **POSITIVE EXPECTED RETURN**: Median projection shows modest gains")
        else:
            insights.append("🚨 **NEGATIVE EXPECTED RETURN**: Median projection shows losses")
        
        # Volatility assessment
        upside_95 = mc_results['percentiles'][95][-1]
        downside_5 = mc_results['percentiles'][5][-1]
        volatility_range = ((upside_95 - downside_5) / starting_balance) * 100
        
        if volatility_range > 200:
            insights.append("⚠️ **VERY HIGH VOLATILITY**: Wide range of possible outcomes (>200%)")
        elif volatility_range > 100:
            insights.append("⚠️ **HIGH VOLATILITY**: Significant uncertainty in outcomes (100-200%)")
        elif volatility_range > 50:
            insights.append("📊 **MODERATE VOLATILITY**: Reasonable range of outcomes (50-100%)")
        else:
            insights.append("✅ **LOW VOLATILITY**: Relatively predictable outcomes (<50% range)")
        
        # Historical performance comparison
        if len(df) > 0:
            historical_return = (df[pnl_col].sum() / starting_balance) * 100
            if expected_return > historical_return * 1.2:
                insights.append("📈 **OPTIMISTIC PROJECTION**: Simulation suggests better future performance")
            elif expected_return < historical_return * 0.8:
                insights.append("📉 **CONSERVATIVE PROJECTION**: Simulation suggests declining performance")
            else:
                insights.append("📊 **CONSISTENT PROJECTION**: Simulation aligns with historical performance")
        
        # Sample size assessment
        num_trades = len(df)
        if num_trades < 30:
            insights.append("⚠️ **LIMITED DATA WARNING**: Results based on <30 trades - increase sample size for reliability")
        elif num_trades < 100:
            insights.append("📊 **MODERATE DATA**: Results based on moderate trade history - more data would improve accuracy")
        else:
            insights.append("✅ **SUFFICIENT DATA**: Results based on substantial trade history")
...
def export_monte_carlo_results(mc_results):
    ...
    try:
        ...
        summary_data = {
            ...
            'Value': [
                mc_results['num_simulations'],
                mc_results['num_periods'],
                f"${mc_results['starting_balance']:,.2f}",
                f"${mc_results['percentiles'][50][-1]:,.2f}",
                f"${mc_results['mean_return']:.2f}",
                f"${mc_results['std_return']:.2f}",
                f"{mc_results['probability_of_profit']:.2f}%",
                f"{mc_results['probability_of_ruin']:.2f}%",
                f"{mc_results['max_drawdown_stats']['median']:.2f}%"
            ]
        }
        
        # Create percentile data
        percentile_data = {'Trade_Number': list(range(mc_results['num_periods'] + 1))}
        for conf in sorted(mc_results['confidence_levels']):
            percentile_data[f'Percentile_{conf}'] = mc_results['percentiles'][conf]
        ...
        # Create CSV string
        output = io.StringIO()
        output.write("MONTE CARLO SIMULATION RESULTS\n")
        output.write("=" * 50 + "\n\n")
        output.write("SUMMARY STATISTICS:\n")
        summary_df.to_csv(output, index=False)
        output.write("\n\nPERCENTILE DATA:\n")
        percentile_df.to_csv(output, index=False)
...
def export_monte_carlo_chart_data(mc_results):
    """Export Monte Carlo chart data for external analysis"""
    try:
        import pandas as pd
        
        # Create chart data
        chart_data = {'Trade_Number': list(range(mc_results['num_periods'] + 1))}
        
        # Add all percentiles
        for conf in [5, 25, 50, 75, 95]:
            if conf in mc_results['percentiles']:
                chart_data[f'P{conf}'] = mc_results['percentiles'][conf]
        
        # Add confidence levels
        for conf in mc_results['confidence_levels']:
            if conf in mc_results['percentiles']:
                chart_data[f'Confidence_{conf}'] = mc_results['percentiles'][conf]
        
        df = pd.DataFrame(chart_data)
        return df.to_csv(index=False)
        
    except Exception as e:
        return f"Error exporting chart data: {str(e)}"
...
def create_monthly_returns_chart(monthly_returns):
    ...
    try:
        if not monthly_returns or len(monthly_returns) == 0:
            return None
